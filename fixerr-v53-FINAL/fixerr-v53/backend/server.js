...
app.post('/api/ads/inquire', auth, async (req,res) => {
  try {
    ...
    const r = await query(
      `INSERT INTO ad_leads (company,name,email,phone,tier,category,message,status,user_id) VALUES ($1,$2,$3,$4,$5,$6,$7,'pending',$8) RETURNING id`,
      [b.company||'', b.name||'', b.email||'', b.phone||'', b.tier||'', b.category||'', b.message||'', req.user.id]
    );
