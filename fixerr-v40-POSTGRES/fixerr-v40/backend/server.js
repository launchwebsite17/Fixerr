...
app.get('/api/pro/bookings', auth, async (req,res) => {
  try {
    ...
    const u = ur.rows[0];
    if(!u) return res.status(404).json({error:'User not found.'});
    const pr = await query('SELECT id FROM pros WHERE user_id=$1',[req.user.id]);
    const proId = pr.rows[0]?.id;
    const fullName = ((u.first||'')+' '+(u.last||'')).toLowerCase().trim();
    const r = await query(
      `SELECT * FROM requests WHERE assigned_pro_id=$1 OR preferred_pro_id=$1 OR preferred_pro_id=$2 OR LOWER(preferred_pro)=$3
       ORDER BY created DESC`,
      [req.user.id, proId||0, fullName]
    );
    res.json(r.rows);
  } catch(e){ console.error(e); res.status(500).json({error:'Could not load bookings.'}); }
});
...
app.listen(PORT, () => {
  ...
  console.log(`   Admin login: admin@fixerr.com / (ADMIN_PASSWORD env var)`);
  console.log(`   If this is a fresh database, run: node migrate.js\n`);
});
