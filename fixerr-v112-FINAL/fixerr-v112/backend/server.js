...
async function sendEmail(to, subject, html, type='general') {
  ...
  try {
    ...
    query(`INSERT INTO email_log (recipient,subject,type,status,error) VALUES ($1,$2,$3,'failed',$4)`,
      [to, subject, type, e.message]).catch(()=>{});
  }
}
...
app.post('/api/auth/register', authLimiter, async (req,res) => {
  try {
    ...
    // Geocode from city+ZIP if GPS not provided by browser
    if((!lat || !lng) && city){
      try {
        const geo = await geocodeCity(zip||city, state, country, zip?city:null);
        if(geo && geo.lat && geo.lng){
          await query('UPDATE users SET lat=$1, lng=$2 WHERE id=$3',[geo.lat, geo.lng, u.id]);
          console.log(`Geocoded user ${u.id} (${city}): ${geo.lat}, ${geo.lng}`);
        }
      } catch(geoErr){ console.log('Geocode skipped:', geoErr.message); }
    } else if(lat && lng){
      console.log(`GPS coords saved for user ${u.id}: ${lat}, ${lng}`);
    }
...
app.get('/api/pros', async (req,res) => {
  try {
    ...
    // Geo-match: use Haversine radius for pros WITH lat/lng
    // Fallback to city name match for pros WITHOUT lat/lng
    if(lat && lng && !isNaN(parseFloat(lat)) && !isNaN(parseFloat(lng))){
      params.push(parseFloat(lat), parseFloat(lng));
      // Show pros that EITHER: are within radius (have geo) OR match city name (no geo yet)
      if(city){
...
app.post('/api/pro-application', auth, async (req,res) => {
  try {
    ...
    if(proUser2.rows[0]){
      ...
      sendEmail(proUser2.rows[0].email, tpl.subject, tpl.html, 'pro_received');
    }
...
app.patch('/api/admin/pros/:id', auth, admin, async (req,res) => {
  try {
    ...
    if(status==='approved'){
      ...
      if(u.rows[0]){ const tpl=EMAIL.proApproved(u.rows[0].first); sendEmail(u.rows[0].email, tpl.subject, tpl.html, 'pro_approved'); }
    }
    res.json({success:true});
...
// ── ADMIN: BACKFILL GEO FOR PROS ─────────────────────────
app.post('/api/admin/backfill-geo', auth, async (req,res) => {
  if(req.user.role!=='admin') return res.status(403).json({error:'Admin only.'});
  try {
    ...
    for(const u of missing.rows){
      try {
        // Try city name directly first (most reliable), then with ZIP
        let geo = await geocodeCity(u.city, u.state, u.country, null);
        if(!geo && u.zip) geo = await geocodeCity(u.zip, u.state, u.country, u.city);
        if(geo && geo.lat && geo.lng){
          await query('UPDATE users SET lat=$1, lng=$2 WHERE id=$3',[geo.lat, geo.lng, u.id]);
          console.log(`Backfilled geo for user ${u.id} (${u.city}): ${geo.lat}, ${geo.lng}`);
          fixed++;
        } else { failed++; console.log(`Geo failed for user ${u.id} city=${u.city}`); }
        // Rate limit Nominatim - max 1 request/second
        await new Promise(r=>setTimeout(r,1200));
      } catch(e){ failed++; console.log(`Geo error for user ${u.id}:`, e.message); }
    }
    res.json({success:true, total:missing.rows.length, fixed, failed});
  } catch(e){ res.status(500).json({error:e.message}); }
});

// ── ADMIN: BACKFILL GEO FOR CUSTOMERS ─────────────────────────
app.post('/api/admin/backfill-geo-customers', auth, async (req,res) => {
  if(req.user.role!=='admin') return res.status(403).json({error:'Admin only.'});
  try {
    const missing = await query(`
    ...
    for(const u of missing.rows){
      try {
        ...
        if(geo && geo.lat && geo.lng){
          await query('UPDATE users SET lat=$1, lng=$2 WHERE id=$3',[geo.lat, geo.lng, u.id]);
          console.log(`Customer geo backfill user ${u.id} (${u.city}): ${geo.lat}, ${geo.lng}`);
          fixed++;
        } else { failed++; }
        await new Promise(r=>setTimeout(r,1200));
      } catch(e){ failed++; }
...
// ── ADMIN: EMAIL LOG ──────────────────────────────────────
app.get('/api/admin/email-log', auth, async (req,res) => {
  if(req.user.role!=='admin') return res.status(403).json({error:'Admin only.'});
  try {
...
app.post('/api/admin/test-email', auth, async (req,res) => {
  ...
  sendEmail(testTo,
    'Fixerr Email Test ✅',
    `<div style="font-family:sans-serif;padding:2rem;"><h2 style="color:#3D8B37;">Email is working!</h2><p>This is a test email from Fixerr at ${new Date().toISOString()}</p><p>ZOHO_EMAIL: ${process.env.ZOHO_EMAIL||'not set'}</p></div>`,
    'test'
  );
});
...
async function initDB() {
  try {
    ...
    const migrations = [
      ...
      // Email log table for tracking sent/failed emails
    ...
    const existing = await query("SELECT id FROM users WHERE email='admin@getfixerr.com'");
