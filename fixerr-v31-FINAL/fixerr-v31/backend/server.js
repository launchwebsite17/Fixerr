...
app.post('/api/auth/login', authLimiter, (req,res) => {
  ...
  // Role check: if tab selected, validate role matches
  if(expected_role && expected_role !== 'admin' && u.role !== expected_role && u.role !== 'admin'){
    const friendly = expected_role==='professional' ? 'professional' : 'customer';
    const actual = u.role==='professional' ? 'professional account' : 'customer account';
    return res.status(403).json({error:`This email is registered as a ${actual}. Please use the ${u.role==='professional'?'Professional':'Customer'} tab to log in.`});
  }
...
// Customer updates their own booking (cancel or approve price) — by ref
