...
function seed() {
  return {
    users: [{
      ...
      hash: bcrypt.hashSync(process.env.ADMIN_PASSWORD || 'CwSNbGdlr1Jl#ESL',10), role:'admin',
...
const authLimiter = rateLimit(15 * 60 * 1000, 30, 'auth');      // 30 attempts / 15 min (raised for testing)
...
app.post('/api/auth/login', authLimiter, (req,res) => {
  ...
  // Role check: admin can login from any tab; others must match
...
app.get('/api/auth/me', auth, (req,res) => {
  ...
  const {hash,...safe} = u;
  // Attach pro status if professional
  if(u.role==='professional'){
    const pro = db.pros.find(p=>p.user_id===u.id);
...
app.get('/api/pros', (req,res) => {
  ...
  const db = load();
  let pros = db.pros
    .filter(p => p.status === 'approved' && p.available)
    .map(p => {
  ...
  // Filter by country
  if(country) pros = pros.filter(p => p.country === country);
  // Filter by city (case-insensitive partial match)
  if(city) pros = pros.filter(p => p.city.toLowerCase().includes(city.toLowerCase()));
  // Filter by category/service
  if(category && category !== 'all') pros = pros.filter(p =>
    Array.isArray(p.services) && p.services.includes(category)
  );
  res.json(pros);
});

// ── BOOKING REQUESTS ──────────────────────────────────────
app.post('/api/requests', bookingLimiter, optionalAuth, (req,res) => {
  ...
  const r = {
    ...
    preferred_pro_id:preferredProId||null,
...
app.get('/api/requests', auth, (req,res) => {
  ...
  // Return requests with assigned pro info for customer dashboard
  const reqs = db.requests.filter(r=>r.user_id===req.user.id).sort((a,b)=>new Date(b.created)-new Date(a.created));
  const enriched = reqs.map(r => {
    let pro_info = {};
    if(r.assigned_pro_id){
      const proUser = db.users.find(u=>u.id===r.assigned_pro_id);
      if(proUser) pro_info = {assigned_pro_name: proUser.first+' '+proUser.last, assigned_pro_phone: r.status==='confirmed'?proUser.phone:null};
    }
    return {...r, ...pro_info};
  });
  res.json(enriched);
...
// Admin updates booking (status, assign pro, propose price)
app.patch('/api/requests/:id', auth, admin, (req,res) => {
  ...
  // Recalculate commission from final price when set
  if(updates.proposed_price || updates.confirmed_price){
    const finalPrice = updates.confirmed_price || updates.proposed_price || db.requests[i].confirmed_price || db.requests[i].proposed_price;
    if(finalPrice){
      updates.commission = Math.round(finalPrice * 0.15);
      updates.pro_earns = Math.round(finalPrice * 0.85);
    }
  }
  // Auto-set review_requested when status becomes completed
  if(updates.status === 'completed') updates.review_requested = true;
  db.requests[i] = {...db.requests[i], ...updates};
  save(db);
  res.json({success:true, commission: db.requests[i].commission, pro_earns: db.requests[i].pro_earns});
});

// Pro: get bookings assigned to this professional
app.get('/api/pro/bookings', auth, (req,res) => {
  const db = load();
  const u = db.users.find(u=>u.id===req.user.id);
  if(!u) return res.status(404).json({error:'User not found.'});
  const proRecord = db.pros.find(p=>p.user_id===req.user.id);
  const fullName = ((u.first||'')+' '+(u.last||'')).toLowerCase().trim();
  const bookings = db.requests.filter(r =>
    r.assigned_pro_id === req.user.id ||
    r.preferred_pro_id === req.user.id ||
    (proRecord && r.preferred_pro_id === proRecord.id) ||
    (r.preferred_pro && r.preferred_pro.toLowerCase() === fullName)
  ).sort((a,b)=>new Date(b.created)-new Date(a.created));
  res.json(bookings);
});

// Pro: update booking status (accept/decline/mark started/mark complete)
app.patch('/api/pro/bookings/:ref', auth, (req,res) => {
  const db = load();
  const u = db.users.find(u=>u.id===req.user.id);
  if(!u||u.role!=='professional') return res.status(403).json({error:'Professionals only.'});
  const i = db.requests.findIndex(r=>r.ref===req.params.ref);
  ...
  const updates={status:req.body.status, updated:new Date().toISOString()};
...
app.patch('/api/requests/ref/:ref', auth, (req,res) => {
  const db = load();
  const i = db.requests.findIndex(r => r.ref===req.params.ref && r.user_id===req.user.id);
  if(i===-1) return res.status(404).json({error:'Booking not found.'});
  ...
  if(status==='cancelled' && !['pending','price_proposed'].includes(db.requests[i].status))
    return res.status(400).json({error:'This booking cannot be cancelled at this stage.'});
  db.requests[i] = {...db.requests[i], ...(status?{status}:{}), ...(confirmed_price?{confirmed_price}:{}), updated: new Date().toISOString()};
  save(db);
  res.json({success:true});
});
...
app.post('/api/reviews', auth, (req,res) => {
  ...
  const existing = db.reviews.find(r=>r.booking_ref===booking_ref&&r.customer===req.user.id);
  ...
  const bi = db.requests.findIndex(r=>r.ref===booking_ref);
  if(bi!==-1) db.requests[bi].review_done = true;
  // Update pro rating
  if(pro_id){
...
app.post('/api/ads/inquire', (req,res) => {
  ...
  db.ad_leads.push({id, ...req.body, status:'pending', created: new Date().toISOString()});
  db.notifs.push({id:uid(db,'notifs'), type:'ad_lead', msg:`New ad inquiry from ${req.body.company||'unknown'}`, read:0, created:new Date().toISOString()});
  save(db);
  res.json({success:true, inquiry_id:id, message:'Inquiry received! We will contact you within 24 hours.'});
});

// Advertiser: get their own inquiries by email
app.get('/api/ads/my', (req,res) => {
  const {email} = req.query;
  if(!email) return res.status(400).json({error:'Email required.'});
  const db = load();
  const leads = db.ad_leads.filter(l=>l.email===email).sort((a,b)=>new Date(b.created)-new Date(a.created));
  res.json(leads);
});

// Admin: update ad lead status + price
app.patch('/api/admin/ad-leads/:id', auth, admin, (req,res) => {
  const db = load();
  const i = db.ad_leads.findIndex(l=>l.id===+req.params.id);
  if(i===-1) return res.status(404).json({error:'Not found.'});
  db.ad_leads[i] = {...db.ad_leads[i], ...req.body, updated: new Date().toISOString()};
  save(db);
  res.json({success:true});
});
...
// Admin DB viewer — returns sanitized db for testing/debugging
app.get('/api/admin/db-view', auth, admin, (req,res) => {
  const db = load();
  const {collection='users'} = req.query;
  const allowed = ['users','pros','requests','messages','reviews','ads'];
  if(!allowed.includes(collection)) return res.status(400).json({error:'Invalid collection'});
  let data = db[collection]||[];
  // Sanitize: remove password hashes from users
...
app.listen(PORT, () => {
  ...
  console.log(`  🔐  Admin login is set via ADMIN_PASSWORD env var (or default if not set)`);
