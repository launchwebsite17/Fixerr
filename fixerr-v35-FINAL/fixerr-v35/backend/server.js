...
app.get('/api/pro/bookings', auth, (req,res) => {
  ...
  const u = db.users.find(u=>u.id===req.user.id);
  if(!u) return res.status(404).json({error:'User not found.'});
  const proName = (u.first+' '+u.last).toLowerCase();
  // Match by assigned_pro_id OR preferred_pro name
  const bookings = db.requests.filter(r =>
    r.assigned_pro_id === req.user.id ||
    (r.preferred_pro && r.preferred_pro.toLowerCase().includes(proName.split(' ')[0]))
