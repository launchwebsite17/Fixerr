...
CREATE TABLE IF NOT EXISTS pros (
  ...
  doc2_type TEXT,
  doc2_url TEXT,
  evaluation_fees_deducted NUMERIC DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
  created_by INTEGER,
  updated_by INTEGER
...
-- State tax rate table for US states and India
...
CREATE TABLE IF NOT EXISTS invoices (
  ...
  user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
  pro_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_invoices_ref ON invoices(booking_ref);
