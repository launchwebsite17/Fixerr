...
// Invoice download links embedded in emails can't carry an Authorization header, so this
// accepts EITHER a normal Bearer session (delegates to `auth`, unchanged for the dashboards)
// OR a `?token=` query param — a short-lived JWT (see emailService.js buildInvoiceViewLink)
// scoped to exactly one booking ref via { scope: 'invoice_view', bookingRef }. A valid link
// token only ever authorizes reading that single booking's invoice — nothing else — via
// req.invoiceLinkBookingRef, which invoiceController.js's canAccessBooking checks explicitly.
async function invoiceLinkOrAuth(req, res, next) {
  ...
  if (token) {
    try {
      ...
      if (decoded.scope === 'invoice_view' && decoded.bookingRef === req.params.bookingRef) {
        req.invoiceLinkBookingRef = decoded.bookingRef;
        return next();
      }
    } catch (err) {
...
module.exports = { auth, optionalAuth, admin, invoiceLinkOrAuth };
