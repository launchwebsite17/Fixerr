...
async function migrate() {
  ...
  for (const u of old.users || []) {
    const r = await pool.query(
      `INSERT INTO users (first,last,email,phone,hash,role,city,state,country,currency,zip,address,active,flagged,flagged_reason,cancellation_count,created_at)
  ...
  for (const p of old.pros || []) {
    ...
    const r = await pool.query(
      `INSERT INTO pros (user_id,status,services,experience_map,years_exp,languages,bio,certifications,photo_url,
         ...
         commission_agreed,liability_agreed,conduct_agreed,has_insurance,has_tools,rating,reviews,badge,available,flagged,created_at)
  ...
  for (const req of old.requests || []) {
    ...
    await pool.query(
      `INSERT INTO requests (ref,user_id,service_key,sub_service,custom_desc,preferred_pro,preferred_pro_id,
         ...
         proposed_price,confirmed_price,status,pro_note,review_requested,review_done,created_at,updated_at)
