...
const appUrl = (pathName = '') => `${APP_BASE_URL}/${String(pathName).replace(/^\/+/, '')}`;

// Email links can't carry an Authorization header, so a "View Invoice" link in an email embeds
// a short-lived, single-purpose signed token instead of requiring the recipient to already be
// logged in. The token only ever grants read access to this one booking's invoice download
// route (see middleware/auth.js's invoiceLinkAuth) — nothing else.
function buildInvoiceViewLink(bookingRef) {
  const token = jwt.sign({ bookingRef, scope: 'invoice_view' }, env.JWT_SECRET, { expiresIn: '30d' });
  return appUrl('api/invoices/' + encodeURIComponent(bookingRef) + '/download') + '?token=' + encodeURIComponent(token);
}

// Embed the logo as a proper email ATTACHMENT (Content-ID / "cid" embedding) rather than a
// base64 data URI in the HTML — Gmail and several other clients don't reliably render inline
// data URIs, but CID-attached images are the traditional, universally-supported way to embed
// images in email. Read once at startup from the same asset the website uses. Falls back to a
// plain text wordmark (never throws) if the file can't be found in a given deployment environment.
const LOGO_PATH = path.join(__dirname, '..', '..', 'frontend', 'assets', 'logo-email.png');
const LOGO_CID = 'fixerr-logo';
let LOGO_BUFFER = null;
try {
  LOGO_BUFFER = fs.readFileSync(LOGO_PATH);
} catch (e) {
  console.error('[emailService] Could not load logo for email embedding from', LOGO_PATH, '-', e.message);
}

// Returns a promise so sendEmail can await the write, guaranteeing every send attempt is logged
// to email_log before sendEmail's own promise resolves — regardless of whether the caller awaits
// sendEmail itself (most callers now fire-and-forget it; this ensures the log still lands).
...
function getZohoTransport() {
  ...
  _zohoTransport = nodemailer.createTransport({
    ...
    // Without these, nodemailer's multi-minute defaults let a slow/unresponsive recipient mail
    // server (e.g. a bad or unreachable domain) hold the awaited sendEmail() call open for a
    // long time — which directly delays the HTTP response on signup/registration.
...
function wrapEmail(contentHtml) {
  const logoHtml = LOGO_BUFFER
    ? `<img src="cid:${LOGO_CID}" alt="Fixerr" style="height:128px;object-fit:contain;display:block;">`
    : `<span style="font-family:'Segoe UI',Helvetica,Arial,sans-serif;font-weight:800;font-size:20px;color:#15803d;">Fixerr</span>`;
...
const EMAIL = {
  welcome: (name, role) => {
    ...
    const ctaHref = appUrl(isPro ? 'dashboard-pro.html' : 'booking.html');
  ...
  proApproved: (name) => ({
    subject: `Your Professional Application Has Been Approved ☑️`,
    html: wrapEmail(`
      ...
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="${appUrl('login.html')}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">Log In to Dashboard →</a>
      </div>
    `)
  }),
  bookingConfirmed: (name, service, date, bookingReference) => ({
    ...
    html: wrapEmail(`
      ...
      <p style="color:#374151;font-size:15px;"><strong>Your Booking Reference #:</strong> ${bookingReference}</p>
      ...
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="${appUrl('dashboard.html')}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">View My Bookings →</a>
      </div>
    `)
  }),
  technicianOnTheWay: (name, proName, proPhone, address, bookingReference) => ({
    ...
    html: wrapEmail(`
      <h2 style="color:#15803d;margin-top:0;">Hi ${name}, your technician is on the way!</h2>
      <p style="color:#374151;font-size:15px;">Fixerr technician <strong><em>${proName}</em></strong> is on the way to your service address. You may contact him if you have any questions.</p>
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;margin:20px 0;">
        <p style="margin:4px 0;color:#166534;"><strong>Booking Reference #:</strong> ${bookingReference}</p>
        <p style="margin:4px 0;color:#166534;"><strong>Technician:</strong> ${proName}</p>
        <p style="margin:4px 0;color:#166534;"><strong>Contact:</strong> ${proPhone}</p>
        <p style="margin:4px 0;color:#166534;"><strong>Address:</strong> ${address}</p>
      </div>
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="${appUrl('dashboard.html')}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">View My Booking →</a>
      </div>
      <p style="color:#374151;font-size:15px;">Thank you!<br>Fixerr Team</p>
    `)
  }),
  bookingAcceptedByPro: (name, proName, proPhone, service, date, priceLine, bookingReference) => ({
    subject: `Your service is confirmed ✅`,
    html: wrapEmail(`
      <h2 style="color:#15803d;margin-top:0;">Good news, ${name} — your service is confirmed! </h2>
      ...
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="${appUrl('dashboard.html')}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">View Booking →</a>
      </div>
    `)
  }),
  ...
  invoiceEmail: (name, invoiceNum, total, currency, pdfHtml, bookingReference, paymentStatus) => {
    const isPaid = paymentStatus === 'paid';
    return {
    subject: `Invoice for your Fixerr Service 🧾`,
    html: wrapEmail(`
      <h2 style="color:#15803d;margin-top:0;">Your Invoice Is Ready${isPaid ? '' : ' - Payment Pending !'}</h2>
      <p style="color:#374151;font-size:15px;">Dear ${name},</p>
      <p style="color:#374151;font-size:15px;">Thank you for choosing Fixerr. Here is your official service invoice details.</p>
      <div style="background:#f9fafb;border:1px solid #e5e7eb;padding:16px;border-radius:8px;margin:20px 0;">
        <p style="margin:4px 0;"><strong>Booking Reference #:</strong> ${bookingReference}</p>
        <p style="margin:4px 0;"><strong>Invoice No:</strong> ${invoiceNum}</p>
        <p style="margin:4px 0;"><strong>${isPaid ? 'Total Paid' : 'Total Amount Due'}:</strong> ${currency === 'INR' ? '₹' : '$'}${total}</p>
        <p style="margin:4px 0;"><strong>Status:</strong> <span style="color:${isPaid ? '#15803d' : '#d97706'};font-weight:bold;">${isPaid ? 'PAID' : 'NOT PAID'}</span></p>
      </div>
      <p style="color:#4b5563;font-size:14px;">You can view and download your full invoice anytime from your Fixerr dashboard.</p>
      <div style="text-align:center;margin:24px 0 8px;">
        <a href="${appUrl('dashboard.html')}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">View Dashboard →</a>
      </div>
    `)
    };
  },
  ...
  ,
    ...
    html: wrapEmail(`
      ...
      <p style="color:#374151;font-size:15px;">Hi ${data.customerName || 'there'},</p>
      <p style="color:#374151;font-size:15px;">We've received your payment for booking <strong>${data.bookingRef}</strong>.</p>
      <div style="background:#f9fafb;border:1px solid #e5e7eb;padding:16px;border-radius:8px;margin:20px 0;">
        <p style="margin:4px 0;"><strong>Booking:</strong> ${data.bookingRef}</p>
        <p style="margin:4px 0;"><strong>Amount Paid:</strong> ${data.currency === 'INR' ? '₹' : '$'}${data.amount}</p>
        ${data.cardBrand && data.cardLast4 ? `<p style="margin:4px 0;"><strong>Payment Method:</strong> ${data.cardBrand.charAt(0).toUpperCase() + data.cardBrand.slice(1)} ending in ${data.cardLast4}</p>` : ''}
        <p style="margin:4px 0;"><strong>Status:</strong> <span style="color:#15803d;font-weight:bold;">PAID</span></p>
      </div>
      <div style="text-align:center;margin:24px 0 8px;">
         <a href="${buildInvoiceViewLink(data.bookingRef)}" style="background:#15803d;color:#fff;padding:12px 24px;border-radius:50px;text-decoration:none;display:inline-block;font-weight:700;">View Invoice →</a>
      </div>
    `)
  })
  ,
  noProfessionalFound: (name) => ({
    subject: `No Professional Available 🔕`,
    html: wrapEmail(`
      <h2 style="color:#15803d;margin-top:0;">Hi ${name},</h2>
      <p style="color:#374151;font-size:15px;">Thank you for choosing <strong>Fixerr</strong>.</p>
      <p style="color:#374151;font-size:15px;">Unfortunately, we couldn't find an available professional within your selected service area for your requested service at this time.</p>
      <p style="color:#374151;font-size:15px;">We're sorry for the inconvenience 🙁. You may try again later or expand your service area to find more available professionals.</p>
      <p style="color:#374151;font-size:15px;">Thank you for your understanding.</p>
    `)
  })
